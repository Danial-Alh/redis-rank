...
describe('Basic leaderboard', () => {
    ...
    const checkCommon = () => {
        ...
        test("check scores", async () => {
            ...
            expect(await lb.score("bar")).toBe(15);
        ...
        test("incr", async () => {
            expect(await lb.incr("bar", 30)).toBe(45); // existing
        ...
        test("improve", async () => {
            ...
            expect(await lb.improve("bar", 15)).toBe(false); // existing equal
    ...
    describe('low to high', () => {
        beforeEach(async () => {
            lb = new TimestampedLeaderboard(rc, { lowToHigh: true });
            await sampleData();
        });
